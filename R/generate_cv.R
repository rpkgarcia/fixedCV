...
#' @keywords internal
#' @noRd
simulate_f_stat <- function(big_T = 1000, d = 1, the_kernel = bartlett, q=1,
                            lugsail_type = "Mother", new_b = b){
  ...
  all_autocovariances[1:d^2, 1:(max_lag+1)] <-sapply(0:max_lag, R,
                                                     the_sim_data = sim_data,
                                                     big_T = big_T)
...
#'
#' Generates fixed-b critical values for robust hypothesis testing. Simulate from
#' a fixed-b distribution to get custom critical values.
#'
#' @param b Numeric vector of bandwidth values between 0 and 1. The bandwidth
...
#' Default is 1.
#' @param alpha Numeric vector of desired significance levels between 0 and 1.
#' Default is 0.05.
#' @param the_kernel A function for the kernel used to generate the weights. The
#' package supports Bartlett (\code{bartlett}), Parzen (\code{parzen}),
#' Tukey-Hanning (\code{th}), or Quadratic Spectral \code{qs}.
#' User supplied functions are also supported.
#' @param lugsail_type Character string specifying the lugsail transformation. Options are
#'   \code{"Mother"} (default), \code{"Zero"}, or \code{"Over"}.
#' @param q The Parzen characteristic exponent of the kernel supplied. The default is 1.
#' @param return_F_stats A logical value indicating if test statistics used to
#' approximate the fixed-b distribution should be returned. The default value is FALSE.
#' @param num_replicates A integer indicating of test statistics to be generated used to
#' approximate the fixed-b distribution. Default is 50,000.
#' @param replicate_size A integer indicating the sample size used to construct
#' the test statistics that are used to approximate the fixed-b distribution. Default is 1000.
#'
#'
#' @details
...
#'  \code{num_replicates} or \code{replicate_size}, especially for higher dimensions.
#'
#'
#' @return If \code{return_F_stats = FALSE} (default) a matrix is returned where the rows
#' correspond to the values for \code{b} and the columns correspond to the
#' significance levels (\code{alpha}) supplied by the user. If \code{return_F_stats = TRUE},
...
generate_cv <- function(b, d = 2, alpha = 0.05, the_kernel = bartlett,
  ...
  if(d == 1){
    results <- generate_cv_uni(b, alpha, the_kernel, lugsail_type, q=1,
                               return_F_stats, replicate_size, num_replicates)
  } else{
    results <- generate_cv_multi(b, d, alpha, the_kernel, lugsail_type, q=1,
                                 return_F_stats, replicate_size, num_replicates)
  }
  return(results)
}
