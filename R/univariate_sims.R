...
# all_autocovariances <- sum(sapply(0:(big_T-1),
...
# sometimes not PSD (computationally!)  - floating pint errors e.g. mess up computation
# package: matrix (function for this issue)
# will need steps to handle non-PSD/certain kernels

# Over lugsail has some issues (optimal b)
#  could have a 0 type 1 error, but can also have
# converge from below
# Not converging fast enough unless T and rho are large (or both large)
# Over lugsails are an afterthought (doesn't make sense for small rho/T)

# When write functions in R:
# utility: real-life warning, add some in for functions



# ## Cochrane & Orcutt procedure for OLS estimation
# # Not needed, but it's cool! Modifies the OLS to account for correlated errors
#
# # The function [ols_ar] iteratively estimates the AR(p) parameters and updates
# # the regression coefficients until convergence.
#
# # Define the OLS function for time series regression with AR(p) errors
# ols_ar <- function(y, X, p = 1, tol = 1e-5, max_iter = 100) {
#   # Add a column of ones to the predictor matrix for the intercept term
#   X <- cbind(1, X)
#
...
#   residuals <- y - X %*% beta
#
#   # Iteratively estimate AR(p) parameters and update beta
#   for (iter in 1:max_iter) {
#     # Estimate AR(p) parameters
#     ar_model <- ar(residuals, order.max = p, method = "yule-walker")
#     phi <- ar_model$ar
#
#     # Adjust y and X for AR(p) errors
#     y_adj <- y[(p+1):length(y)]
#     X_adj <- X[(p+1):nrow(X), ]
#     for (i in 1:p) {
#       y_adj <- y_adj - phi[i] * y[(p+1-i):(length(y)-i)]
#       X_adj <- X_adj - phi[i] * X[(p+1-i):(nrow(X)-i), ]
#     }
#
#     # Update beta
#     beta_new <- solve(t(X_adj) %*% X_adj) %*% t(X_adj) %*% y_adj
#
